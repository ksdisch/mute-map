...
provenance table is the map. No figures exist in this repository — every number below is
a table or an inline value.*
...
| M1 gate | +0.656 [+0.517, +0.763] / +0.636 [+0.443, +0.759] | `results/m1-battery-*.json` → `breadth_contrast.newcombe_control_minus_primed_late_naming` |
...
| M2 gate | +0.853 / +0.794 at 1.5B; +0.750 / +0.688 at 3B | `results/m2-depth-*.json` → `localization_contrast` |
| M2 window sweep | 1.5B: 33,32,27,28,28,27,25,23,23,1,**0**,0 of 34 | same → `window_map[*].cell` |
| M2 dose curve | 1.5B 34,20,3,1,0 of 34 (mass .913,.594,.115,.037,.017) | same → `dose_curve` |
| Half-mute λ ≈ 0.23/0.29/0.36 | **INTERPOLATED, not measured** | `docs/M2-BRIEF.md` — the brief says so itself |
...
| M3 clause (1) | +0.971 [+0.867, +0.983] / +0.881 [+0.731, +0.943] | `results/m3-matrix-*.json` → `specificity_contrast.clause_1_pooled` |
| M3 clause (2) | +0.950 [+0.814, +0.978] / +0.891 [+0.730, +0.947] | same → `clause_2_within_category` |
...
| **M4 headline** | 11/41 = 0.268; **51/71 = 0.718 [0.605, 0.810]**; **63/84 = 0.750 [0.648, 0.830]** | same → `vocabulary_sparing.gate_arm` |
| **M4 concept-level (the honest quote)** | **24/41 = 0.585, lower 0.434**; **26/43 = 0.605, lower 0.456** | same → `vocabulary_sparing.conservative_reads[concept-level]` |
| M4 verdict strings, verbatim | `VOCAB-SPARING (…) — AS-SCORED ONLY (concept-level …)` | same → `vocabulary_sparing.verdict` |
| Residual-conservative read (clears) | 49/71 lower 0.575; 62/84 lower 0.635 | same → `conservative_reads[residual-conservative]` |
| Two cross-checks, two generations | M1 255/255 and M3 468/468 cells, mass exact, ×3 | same → `m1_crosscheck`, `m3_crosscheck` |
| No prime is a wrecking ball | 1.5B rows 63/71–67/71; 3B 77/84–83/84 | same → `row_profiles[*].collateral_non_subset` |
| Fragile columns | `copper` 6/12, `mosquito` 8/12; `eagle` 8/12, `platinum` 9/12 | same → `column_profiles[*].fragility` |
...
| Cost, suite | $0; 396 tests green | briefs' wall-clock sections; `uv run pytest` |
...
by the runner, conditionally, not written by me after seeing the result.
...
button, and I genuinely don't know the answer. Nearer term: the recorded JSONs already
contain the per-pair cells a figures pass would draw — the 12 × 12 matrix, the dose curve,
the strip's fragile-probe columns — which is a plotting job over committed data, not a
new measurement.
